...
There are two type of errors that can be made with a statistical test:

-   **Type I error**: [rejecting $H_0$ when it is true]{.hi-pink}. False-positive result. It has a probability of $\alpha$ which is the statistical significance level.
...
plot_error(quant_norm = 0.95, diff = 1, precision = 0.5, title_w = "Increasing Variability")
```
:::
:::
:::
...
-   The statistical power of a test allows to control Type II error rate.
...
-   Compute the sample size using [formula]{.hi-pink} existing for a statistical test with a defined $\alpha$, power and [effect size]{.hi-pink}.

-   This approach is limited to study design and statistical test where formula exist.

-   Using simulations (iterative procedure):

    1.  Fix [$n$]{.hi-pink} and [generate samples]{.hi-pink} under a theoretical model and the parameters value from $H_a$,
...
-   With the same study design and parameters values, what if 1 patient with the best FEV1 is lost to follow-up in the treatment group only?
...
-   Sample size computation allows to control Type II error.

-   It is an important part of the [study plannification]{.hi-pink}.

-   Depending on the complexity of the study design and the applied statistical model it can be done using formula or simulations.
